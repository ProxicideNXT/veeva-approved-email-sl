...
export const SubjectLineList = () => {
  ...
  return (
    <DragDropContext onDragEnd={onDragEnd}>
      ...
      <button className='rounded-md	bg-[#008fd2] text-white p-[15px] mb-[15px]' onClick={onAddSubjectLine}>Add option</button>
        ...
        {provided => (
          ...
          >
            {options.map((option, index) => (
              <SubjectLine key={index} index={index} text={option}></SubjectLine>
            ))}
